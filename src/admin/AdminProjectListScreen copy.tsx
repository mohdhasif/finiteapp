...
const AdminProjectListScreen = () => {
    ...
    const fetchData = async (isRefreshing = false) => {
        try {
            ...
            const result = await getProjects(token); // pastikan return array of Project
                ...
                {filteredProjects.length === 0 ? (
                    ...
                    filteredProjects.map((item) => (
                        <ProjectCardScreen
                            ...
                            total_tasks={(item as any).total_tasks ?? undefined}   // if you have it
                            assignees={(item as any).assignees ?? []}              // optional
                            onPress={() => goToTasks(item)}
                        />
                    ))
                )}
